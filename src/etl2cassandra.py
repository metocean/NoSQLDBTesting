...
descDB = ["10.181.2.50","10.181.2.51", "10.181.3.10", "10.181.3.11"]
...
class Consumer(threading.Thread):
    ...
    def run(self):
        cluster = Cluster(descDB)
        session = cluster.connect()
        bound_statement = session.prepare("""
                INSERT INTO forecast.verify
                (site, cycle, forecastType, forecast)
                  VALUES (?, ?, ?, ?);""")
        ...
        while True:
            ...
            if len(cycles["hits"]["hits"]) == 0:
                ...
                for cycle in cycles["hits"]["hits"]:
                    ...
                    tjson = {}
                    tjson["site"]=site
                    thisCycle=cycle["fields"]["cycle"][0]
                    tjson["cycle"]=thisCycle
                    forecasttype=str(cycle["fields"]["forecastType"][0])
                    tjson["forecastType"]= forecasttype
                    forecast=collection
                    ...
                    result = session.execute(bound_statement.bind((
                        site,
                        datetime.datetime.strptime(thisCycle, '%Y-%m-%dT%H:%M:%S'),
                        forecasttype,
                        forecast)))
...
if __name__ == "__main__":
    ...
    , level=logging.INFO, filename="etl2cassandra.log")
